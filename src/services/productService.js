...
const productService = {
   ...
   */
  getPaginatedProductsByCategory: async (
    category,
    page = 0,
    size = 10,
    sortBy = "productId",
    ...
    try {
      ...
      console.error(
        `Lỗi khi lấy sản phẩm theo danh mục "${category}" có phân trang:`,
        error
      );
      throw error;
    }
   ...
   */
  getPaginatedProductsByPriceRange: async (
    minPrice,
    maxPrice = null,
    page = 0,
    size = 10,
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/by-price-range`, {
        params,
      });
      return response.data;
    } catch (error) {
      console.error(
        `Lỗi khi lấy sản phẩm theo khoảng giá ${minPrice}-${maxPrice} có phân trang:`,
        error
      );
      throw error;
    }
