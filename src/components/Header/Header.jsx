...
const Header = () => {
    ...
    const handleScroll = () => {
        ...
        if (offset > 100) { // Ví dụ: cuộn 100px thì đổi màu
            setScrolled(true);
        } else {
            setScrolled(false);
        }
        ...
        <header className={headerClasses}>
            <div className="header-left">
                <Link to="/" className="logo-link">
                    <div className="logo-image-element" aria-label="Dola Bakery Logo"></div>
            ...
            <nav className="header-nav">
                <ul>
                    ...
                    <li><Link to="/store-system">Hệ thống cửa hàng</Link></li> {/* Thêm link này nếu có */}
                    <li><Link to="/news">Tin tức</Link></li> {/* Thêm link này nếu có */}
                </ul>
            </nav>
            <div className="header-right">
                {/* Các icon tìm kiếm, giỏ hàng, wishlist... */}
                <span className="header-icon"><i className="fas fa-search"></i></span>
                <span className="header-icon"><i className="fas fa-heart"></i></span>
                <span className="header-icon"><i className="fas fa-shopping-cart"></i></span>
            </div>
        </header>
    );
};
